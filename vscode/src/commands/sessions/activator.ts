...
/**
 * Registers all commands of the session module.
 *
 * @export
...
export function activateSessions(extension: SarosExtension) {
  commands.registerCommand('saros.session.connect', async () => {
    ...
    const result =
      await extension.client.sendRequest(ConnectRequest.type, null);
  ...
  commands.registerCommand('saros.session.disconnect', async () => {
    await extension.onReady();
    const result =
      await extension.client.sendRequest(DisconnectRequest.type, null);
  ...
  commands.registerCommand('saros.session.start', async () => {
    await extension.onReady();
    const result =
      await extension.client.sendRequest(StartSessionRequest.type, null);
  ...
  commands.registerCommand('saros.session.stop', async () => {
    await extension.onReady();
    const result =
      await extension.client.sendRequest(StopSessionRequest.type, null);
