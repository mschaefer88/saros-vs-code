...
public class XMPPContactsService implements Disposable {
  ...
  private void handleContactResourceAvailable(XMPPContact contact, JID fullJid, Presence presence) {
    ...
    discoveryService.querySarosSupport(fullJid, handleQuerySarosSupportResult(fullJid, contact));
  }
   ...
   */
  private Consumer<Boolean> handleQuerySarosSupportResult(JID fullJid, XMPPContact contact) {
    return hasSupport -> {
      contactsExecutor.execute(
          () -> {
