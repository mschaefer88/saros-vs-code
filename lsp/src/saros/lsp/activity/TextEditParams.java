...
public class TextEditParams extends ApplyWorkspaceEditParams {
  ...
  public TextEditParams(
      IWorkspace workspace, EditorManager editorManager, TextEditActivity activity) {
  ...
  private static String createFileUri(IWorkspace workspace, SPath path) {
    return Paths.get(workspace.getLocation().append(path.getFullPath()).toString())
        .toUri()
        .toString(); // TODO: do bettter!
  }
  ...
  private static TextEdit createEdit(String content, TextEditActivity activity) {
    ...
    edit.setRange(
        new Range(
            editorString.getPosition(offset),
            editorString.getPosition(offset + activity.getReplacedText().length())));
