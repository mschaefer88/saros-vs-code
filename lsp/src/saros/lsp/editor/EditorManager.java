package saros.lsp.editor; // TODO: best package location?
...
public class EditorManager implements IEditorManager {
  ...
  private Map<SPath, Editor> openEditors =
      Collections.synchronizedMap(new HashMap<SPath, Editor>());
    ...
    try {
      Editor editor =
          this.openEditors.containsKey(path)
              ...
              : this.openEditors.put(path, new Editor(path.getFile()));
  ...
  public void setVersion(SPath path, int version) { // TODO: experimental!
  ...
  public void saveEditor(SPath path) {
    try {
      path.getFile()
          .setContents(IOUtils.toInputStream(this.openEditors.get(path).getText()), true, false);
    } catch (IOException e) {
      LOG.error(e);
    }
  }
